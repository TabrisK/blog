    ...
    <section class="a-detail">
      ...
      <p class="post-meta">
        <span>
          ...
          {{info.releaseTime | date("yyyy-MM-dd")}}
        </span>
        <span>
...
export default {
  ...
  beforeMount() {
    this.getArticleInfo();
    if (this.$route.params.id) article.view({ id: this.$route.params.id }, {});
  },
  mounted() {
    new Valine({
      el: "#vcomments",
      appId: "iyFyoTXDHnczSB6PTgmTdp11-gzGzoHsz",
      appKey: "a7k9NIltqoMurJIobh2r9xlB",
      placeholder: "在这里输入评论",
